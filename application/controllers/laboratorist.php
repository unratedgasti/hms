...

{
...

	{
		...
		if ($this->session->userdata('laboratorist_login') != 1)
			...
			redirect(base_url() . 'index.php?login', 'refresh');
		$this->db->select('pxinfo.recno,pxinfo.pxfname,pxinfo.pxlname,pxrequestque.requestdate');    
		$this->db->from('pxinfo');
		$this->db->join('pxlabrec', 'pxinfo.recno = pxlabrec.pxrecno');

		$this->db->join('pxrequestque', 'pxlabrec.rxrecno = pxrequestque.rxrecno');
		$this->db->where("pxrequestque.servicetype","Laboratory");
		$this->db->group_by('pxinfo.recno'); 
		$query = $this->db->get()->result_array();
		$page_data['patients'] = $query;
	...
	function laboratory(){
		...
		if(!$this->uri->segment(3)){
			...
			$this->db->join('pxrequestque', 'pxlabrec.rxrecno = pxrequestque.rxrecno');
			$this->db->where("pxrequestque.servicetype","Laboratory");
			$this->db->where("pxlabrec.pxrecno",$this->uri->segment(3));

			$page_data['patientresult'] = $this->db->get()->result_array();
			$page_data['page_name']  = 'laboratory';

			$page_data['page_title'] = get_phrase('laboratory');
			$this->load->view('index', $page_data);
		}
	...
	function rme(){
		...
		$page_data['page_name']  = 'rme';

		$page_data['page_title'] = get_phrase('rme');
		$this->load->view('index', $page_data);
	}
	...
	function physicalexam(){
		...
		$page_data['page_name']  = 'physicalexam';

		$page_data['page_title'] = get_phrase('Physical Examination');
		$this->load->view('index', $page_data);
	}
